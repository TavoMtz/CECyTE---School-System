...
-- Corrección: Se cambió id_materia_periodo por id_materiaPeriodo
SELECT 
    h.dia_semana, 
...
-- Corrección: Plantel_id_plantel cambió a id_plantel
SELECT 
...
-- Corrección: id_cuestionario_Docente cambió a id_cuestionarioDocente (sin guion bajo)
SELECT 
    CONCAT(per.nombre, ' ', per.apellido_P) AS Docente,
    m.nombre AS Materia,
    pre.texto_pregunta,
...
JOIN Cuestionario_Docente cd ON ed.id_cuestionarioDocente = cd.id_cuestionarioDocente -- Corregido
JOIN Pregunta pre ON cd.id_cuestionarioDocente = pre.id_cuestionarioDocente -- Corregido
JOIN Respuesta r ON pre.id_pregunta = r.id_pregunta
GROUP BY d.id_docente, m.id_materia, pre.id_pregunta;
...
-- Corrección: id_materia_periodo cambió a id_materiaPeriodo
SELECT 
    concat(p.nombre, ' ', p.apellido_P, ' ', p.apellido_M) AS Docente,
...
-- Corrección: id_plan_Estudios cambió a id_planEstudios
SELECT 
    e.matricula,
